...
def match_id(filename):
    with open('C:/Users/Administrator/Desktop/Image/image_data.json') as json_file:
...
def cluster(path):
    ...
    allfiles = glob.glob('C:/Users/Administrator/Desktop/Image/*.npz')
    ...
    for i in file_index_to_file_name.keys():
        ...
        for j in range(1):
            ...
            app_npz = glob.glob(''+path+'*.npz')
            load_npz = np.loadtxt(app_npz[0])
            similarity = 1 - spatial.distance.cosine(master_vector,load_npz)
            rounded_similarity = int((similarity * 10000)) / 10000.0
            # Appends master product id with the similarity score
            # and the product id of the similar items
            named_nearest_neighbors.append({'similarity': rounded_similarity,'master_pi': master_product_id})
    most_similarity = ''
    most_product_id = ''
    print(named_nearest_neighbors)
    for i in range(len(named_nearest_neighbors)):
        get_similarity = named_nearest_neighbors[i].get("similarity")
        get_product_id = named_nearest_neighbors[i].get("master_pi")
        if i == 0:
            most_similarity = get_similarity
            most_product_id = get_product_id
        else:
            if most_similarity < get_similarity:
                most_similarity = get_similarity
                most_product_id = get_product_id
            else:
                continue
    if most_similarity < 0.8:
        return 'Not Find'
    else:
        return most_product_id
    print("---------------------------------")
    print("Similarity index       : %s" %i)
    print("Master Image file name : %s" %file_index_to_file_name[i])
    print("Nearest Neighbors.     : %s" %nearest_neighbors)
    print("--- %.2f minutes passed ---------" % ((time.time() -start_time)/60))
    return file_index_to_file_name[i]
